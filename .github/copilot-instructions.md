...
In building and testing, never use `dotnet` without extension.
...
(3) To then run tests, use a command similar to this `dotnet.sh test tests/Showcase.Seq.Tests/Showcase.Seq.Tests.csproj` (using the path to whatever projects are applicable to the change).
