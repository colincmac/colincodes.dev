...
public static partial class AIJsonUtilitiesExtensions
{
    ...
    private static readonly byte[] sseEventFieldPrefix = "event: "u8.ToArray();
    private static readonly byte[] sseDataFieldPrefix = "data: "u8.ToArray();
    private static readonly byte[] sseIdFieldPrefix = "id: "u8.ToArray();
    private static readonly byte[] sseLineBreak = Encoding.UTF8.GetBytes("\n");
    ...
    private static async ValueTask SendAsync<T>(Stream stream, SseEvent<T> sseEvent, ArrayBufferWriter<byte> bufferWriter, CancellationToken cancellationToken = default)
    {
        ...
        {
            await stream.WriteAsync(sseEventFieldPrefix, cancellationToken).ConfigureAwait(false);
            await stream.WriteAsync(Encoding.UTF8.GetBytes(eventType), cancellationToken).ConfigureAwait(false);
            await stream.WriteAsync(sseLineBreak, cancellationToken).ConfigureAwait(false);
        }


        await stream.WriteAsync(sseDataFieldPrefix, cancellationToken).ConfigureAwait(false);
        ...
        {
            await stream.WriteAsync(sseIdFieldPrefix, cancellationToken).ConfigureAwait(false);
