...
public class OutputWebSocketHandler : IOutputWebSocket
{
    ...
    public async Task StartProcessingWebSocketAsync(IVoiceClient voiceClient, CancellationToken cancellationToken = default)
    {
        try
        {
    ...
    {
            var requestData = ConvertAudioToRequestData(audio);
            await SendCommandAsync(requestData, cancellationToken);
    }

    public virtual async Task SendStopAudioCommandAsync(CancellationToken cancellationToken = default)
    {
