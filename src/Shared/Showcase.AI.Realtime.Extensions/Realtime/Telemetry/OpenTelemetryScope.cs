...
using static Showcase.AI.Realtime.Extensions.Realtime.Telemetry.OpenTelemetryConstants;
...
{
    private static readonly ActivitySource chatSource = new(DefaultSourceName);
    private static readonly Meter chatMeter = new(DefaultSourceName);
    ...
    private static readonly Histogram<double> operationDurationHistogram = chatMeter.CreateHistogram<double>(
    ...
    private static readonly Histogram<int> tokenUsageHistogram = chatMeter.CreateHistogram<int>(
    ...
    private void StartConversationSession()
    {
        ...
        _activity = chatSource.StartActivity(string.IsNullOrWhiteSpace(_requestModelId) ? GenAI.RealtimeSessionConversation : $"{GenAI.RealtimeSessionConversation} {_requestModelId}", ActivityKind.Client);
        return;
    }
    ...
    private void RecordMetrics(string? responseModel, string? errorType, int? inputTokensUsage, int? outputTokensUsage)
    {
        ...
        if (inputTokensUsage != null)
        {
            ...
            tokenUsageHistogram.Record(inputTokensUsage.Value, inputUsageTags);
        }
        ...
        if (outputTokensUsage != null)
        {
            ...
            tokenUsageHistogram.Record(outputTokensUsage.Value, outputUsageTags);
        }
        ...
        if(_duration != null)
        {
            operationDurationHistogram.Record(_duration.Elapsed.TotalSeconds, tags);
        }
    }
